...
dish_categories = [VEGAN, VEGETARIAN, KETO, PALEO, OMNIVORE]
dish_categories_string = ['VEGAN', 'VEGETARIAN', 'KETO', 'PALEO', 'OMNIVORE']
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    for index, category in enumerate(dish_categories):
        category_name = dish_categories_string[index]
...
def compile_ingredients(dishes):
    ...
    for index, dish in enumerate(dishes):
        if index > 0: new_dishes = set(new_dishes.union(dish))
        else: new_dishes = set(dish)
...
def singleton_ingredients(dishes, intersection):
    ...
    dishes = compile_ingredients(dishes)
    return set(dishes.symmetric_difference(intersection))